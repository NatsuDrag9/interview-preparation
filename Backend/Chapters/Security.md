...
*   **SQL Injection**: If you build database queries by concatenating raw user strings, attackers can inject SQL commands. 
    ...
    *   **The Fix**: Use *Parameterized Queries* (Prepared Statements). This separates the query template from the data. The database treats whatever is inside the parameter slot purely as a string, making it impossible to execute as a command. 
