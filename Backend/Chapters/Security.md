...
### The Core Security Mindset

- **The Problem**: No application is perfectly secure, but many vulnerabilities stem from a single question: _"Where did the developer make an assumption?"_. Developers often assume the "happy path"—that users will type correct inputs and only click intended buttons.
- **The Attacker's Mindset**: Attackers do not follow the happy path. They purposefully break inputs and poke at boundaries. As a backend engineer, you must always ask: _"What could go wrong here in terms of security?"_.
...
- **SQL Injection**: If you build database queries by concatenating raw user strings, attackers can inject SQL commands.
  ...
  - **The Fix**: Use _Parameterized Queries_ (Prepared Statements). This separates the query template from the data. The database treats whatever is inside the parameter slot purely as a string, making it impossible to execute as a command.
...
- **Salting (Crucial)**: To defeat Rainbow Tables, generate a cryptographically random string (a _Salt_) for each user. Concatenate the salt to their password before hashing it. This ensures that even if two users have the same password, their hashes will look completely different.
- **Slow Hashing Functions**: Modern GPU graphics cards can guess billions of fast hashes (like MD5 or SHA-256) per second. You must use _slow hashing functions_ explicitly designed for passwords, such as _Argon2id_ or **Bcrypt**. These include a "cost factor" that forces the calculation to take around 400 milliseconds, slowing down brute-force attacks so much that cracking passwords would take centuries.

### Sessions vs. JWTs (Stateful vs. Stateless Auth)
...
- **Authorization Principles**: Centralize your logic, adopt a _Default Deny_ policy (block everything unless explicitly allowed), and write automated test suites specifically testing edge-case boundary violations.

### Frontend-Facing Vulnerabilities (XSS & CSRF)
...
### Misconfigurations and Defense in Depth
...
1. Client Input Validation (Validate shapes/types via Zod/Joi)
   ⬇
2. Transport Layer Security (Enforce HSTS / HTTPS-only traffic)
   ⬇
3. Network / Gateway Filtering (WAF - Web Application Firewall & Rate Limiting)
   ⬇
4. Application Middleware (Inject Security Headers: CSP, X-Frame-Options, CORS)
   ⬇
5. Data/Query Access Layer (Parameterized inputs & Context-aware Authorization checks)
...
1. Token Defense: Browser sends requests with auth cookies set to HttpOnly + Secure.
   [ API GATEWAY / WAF ]
   │
   ▼
2. Rate Limiting: Redis checks request frequency based on User ID/IP to stop DoS attacks.
   [ BACKEND CONTROLLER ]
   │
   ▼
3. Schema Validation: Request body is parsed via a Validator (like Zod/DTOs).
   │ Unexpected fields (e.g., "isAdmin": true) are instantly dropped.
   │
   ▼
4. Sanitization: If input contains rich text/HTML, it is cleaned via DOMPurify.
   [ BUSINESS LOGIC LAYER ]
   │
   ▼
5. Ownership & Tenancy Check: Code pulls the User ID + Tenant ID directly from the session.
   │ It explicitly injects them into the database query.
   [ DATABASE ]
   │
   ▼
6. Contextual SQL: Executes as "WHERE id = target_id AND tenant_id = X AND user_id = Y".

If asked, _How do you securely design a profile update endpoint?_, you say:

- **On the Frontend**: The browser submits the update request along with an `HttpOnly` cookie so JavaScript cannot access the session token.
- **At the API Gate**: The request goes through a backend validator schema to strip out any unallowed fields like role changes, and rich text fields are sanitized.
- **At the Database**: The update query explicitly binds the logged-in user's User ID and Tenant ID from the session context to prevent unauthorized data access.

Reference - Google Ai-Mode search
