...
- Request reaches the entry point of the server, i.e. the port it is listening on
...
- After repository, before service (when sending a response)
- After service, before handler and; (when sending a response)
- After handler, before sending the response to the client (when sending a response)
...
Without a Request Context, if a deep layer (like the Repository Layer) needs access to the logged-in User's ID or a Correlation ID, you would have to pass these values as arguments through **every single function** from the Controller to the Service to the Repository. This is known as _"prop-drilling"_ and pollutes clean function signatures. Request Context allows layers to share data implicitly.
